...
static const char* s_help3DS_New[] = {
	"~~~~~~~~~~~~~~ HELP / ABOUT ~~~~~~~~~~~~~~",
	"DOOM RPG Reverse Engineered - 3DS Port",
	"Ported by Dennis Isaac Gutierrez Zeledon",
	"",
	"In DOOM RPG you battle monsters, solve puzzles,",
	"and interact with survivors to uncover the truth",
	"behind the demonic invasion on Mars.",
	"",
	"~~~~~~~~~~~~~~~~ GENERAL ~~~~~~~~~~~~~~~~",
	"Circle Pad / D-Pad: Move around and navigate menus",
	"A Button: Talk, attack, and interact with objects",
	"B Button: Pass turn, dismiss dialogs, cancel",
	"START: Open in-game menu",
	"SELECT: Recenter view / camera",
	"",
	"~~~~~~~~~~~~~~~~ CONTROLS ~~~~~~~~~~~~~~~~",
	"CPad / D-Pad: Move forward/back, turn left/right",
	"L / R Shoulder: Strafe left / right",
	"ZL / ZR or X / Y: Switch weapons",
	"Right Nub (C-Stick): Scroll messages & automap",
	"Touch Screen: Direct touch interaction & keypad",
	"",
	"~~~~~~~~~~~~~~~ TOUCH HUD ~~~~~~~~~~~~~~~~",
	"Bottom Hotbar: Quick-use items (medkits, ammo)",
	"Left Bar: Weapon selector & dog companion",
	"Automap: Drag to pan, touch [+] / [-] to zoom",
	"Touch [CTR]: Recenter automap on player",
	"Touch [PASS]: Pass current turn",
	"Touch [TURBO]: Toggle fast-forward animations",
	"Keypad: Tap numbers directly to enter door codes",
	"",
	"~~~~~~~~~~~~~~~~ LEGALS ~~~~~~~~~~~~~~~~~",
	"(c) 2005 id Software, Inc. All rights reserved.",
	"Reverse engineered by GEC team."
};
...
static const char* s_help3DS_Old[] = {
	...
	"CPad / D-Pad: Move forward/back, turn left/right",
	"L / R Shoulder: Strafe left / right",
	"X / Y Buttons: Switch weapons",
	"X / Y or Touch: Scroll dialogs & message logs",
	"Touch Screen: Direct touch interaction & keypad",
	"",
	"~~~~~~~~~~~~~~~ TOUCH HUD ~~~~~~~~~~~~~~~~",
	"Bottom Hotbar: Quick-use items (medkits, ammo)",
	...
	switch (i) {
		...
		case MENU_STORE_BUY: {
			...
			if (i18 == 5) {
				...
				if (curCoins >= 0 && curCoins < 5) {
					strncpy(menu->doomRpg->hud->logMessage, "Need 5 Play Coins", MS_PER_CHAR);
					MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Need 5 Play Coins!", 3, 0);
					MenuItem_Set(&menuSystem->items[menuSystem->numItems++], NULL, 0, 0);
					MenuItem_Set(&menuSystem->items[menuSystem->numItems++], "Back", 2, 0);
					menuSystem->selectedIndex = 2;
					menuSystem->type = 6;
